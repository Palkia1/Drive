...
/**
 * Trimmed down from the @skiper-ui/skiper26 registry component: this file
 * originally shipped a full demo page (fake marketing copy + a draggable
 * "Options" panel for live-tweaking variant/blur/gif, including hardcoded
 * Giphy URLs) — none of which belongs in the shipped app. Kept: the
 * useThemeToggle hook, ThemeToggleButton, and the pure-CSS animation
 * engine, with ThemeToggleButton restyled to Rijklaar's own tokens instead
 * of the original's hardcoded bg-black. The "gif" variant (which needs an
 * external image URL) is intentionally not exposed via our wrapper.
 */
...
export const useThemeToggle = ({
  ...
  // Sync isDark state with resolved theme after hydration — resolvedTheme is
  // undefined during SSR, so this can't be a lazy useState initializer.
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time hydration sync, not a derived-state anti-pattern
  ...
  const toggleTheme = useCallback(() => {
    ...
    const switchTheme = () => {
      // Compare against isDark (derived from resolvedTheme), not the raw
      // `theme` preference — when defaultTheme is "system", `theme` can be
      // "system" rather than "light"/"dark", which made the very first
      // click a no-op instead of a toggle.
...
export const ThemeToggleButton = ({
  ...
  return (
    <button
      ...
      style={{ background: "var(--brand-600)", boxShadow: "0 1px 2px rgba(18, 19, 26, 0.06), 0 6px 14px -8px color-mix(in srgb, var(--brand-600) 65%, transparent)" }}
      onClick={toggleTheme}
